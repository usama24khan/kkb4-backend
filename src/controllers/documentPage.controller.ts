...
export const renderDocumentPage = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (!doc) {
      if (process.env.NODE_ENV !== 'production') res.set('x-kkb4-stage', 'not-found');
      res.status(200).send(
        renderHtml(pageUrl, {
    ...
    if (!doc.pdfAvailable) {
      if (process.env.NODE_ENV !== 'production') res.set('x-kkb4-stage', 'no-pdf');
      res.status(200).send(
        renderHtml(pageUrl, {
          title: doc.title,
    ...
    // Reason echoed in a header outside production, so a failure here is
    // diagnosable without shipping internals to residents.
